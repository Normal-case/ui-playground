...
이것이 바로 회전 변환 공식입니다! ✓`}
      ...
      <Card>
        ...
        <Text size="2" mb="4">
          이제 실제로 회전된 도형을 리사이즈하는 코드를 작성해봅시다. 단계별로
          살펴보겠습니다.
        </Text>
...
const localDelta = globalToLocalCoordinates(
  ...
  shape.rotation
        ...
        <Box mb="4">
          <Heading size="3" mb="2">
            Step 3: 로컬 좌표계 기준으로 크기 계산
          </Heading>
          <Card className="bg-gray-50 dark:bg-gray-900 p-4">
            <pre className="text-sm">
              {`// 리사이즈 핸들에 따라 크기 조정 (로컬 좌표계)
switch (resizeHandle) {
  case 'bottom-right':
    newWidth = initialWidth + localDelta.x
    newHeight = initialHeight + localDelta.y
    break
  case 'top-left':
    newWidth = initialWidth - localDelta.x
    newHeight = initialHeight - localDelta.y
  ...
  // ... 다른 핸들들
        ...
        <Box mb="4">
          ...
          <Text size="2" mb="2">
            크기가 변경되면 중심점이 이동할 수 있습니다. 특정 핸들의 경우 위치도
            함께 조정해야 합니다.
          </Text>
          <Card className="bg-gray-50 dark:bg-gray-900 p-4">
            <pre className="text-sm">
              {`// 크기 변화에 따른 중심점 오프셋 계산 (로컬 좌표계)
const centerOffsetX = (newWidth - initialWidth) / 2
const centerOffsetY = (newHeight - initialHeight) / 2
...
// 위치 업데이트
newX = initialCenterX + globalOffset.x - newWidth / 2
newY = initialCenterY + globalOffset.y - newHeight / 2`}
            </pre>
          </Card>
        </Box>
        ...
        <Callout.Root color="green">
          <Callout.Text>
            <strong>핵심 포인트:</strong> 마우스 입력(전역) → 로컬 변환 → 크기
            계산 → 전역 변환 → 위치 업데이트. 이 흐름을 따라가면 회전된 도형도
            자연스럽게 리사이즈됩니다.
          </Callout.Text>
        </Callout.Root>
      </Card>
...
function handleResize(
  ...
  switch (resizeHandle) {
    case 'top-left':
      centerOffsetX = (initialBounds.width - newWidth) / 2
      centerOffsetY = (initialBounds.height - newHeight) / 2
      break
    case 'top-right':
      centerOffsetX = (newWidth - initialBounds.width) / 2
      centerOffsetY = (initialBounds.height - newHeight) / 2
      break
    case 'bottom-left':
      centerOffsetX = (initialBounds.width - newWidth) / 2
      centerOffsetY = (newHeight - initialBounds.height) / 2
      break
    case 'bottom-right':
      centerOffsetX = (newWidth - initialBounds.width) / 2
      centerOffsetY = (newHeight - initialBounds.height) / 2
  ...
  // 최종 위치 계산
  const newX = initialCenter.x + globalOffset.x - newWidth / 2
  const newY = initialCenter.y + globalOffset.y - newHeight / 2
      ...
      <Card>
        ...
        <Box className="space-y-4">
          <Box>
            ...
            <Text size="2" mb="2">
              회전된 도형이 캔버스를 벗어나지 않도록 하려면 회전된 꼭짓점을
              기준으로 바운딩 박스를 계산해야 합니다.
            </Text>
            <Card className="bg-gray-50 dark:bg-gray-900 p-4">
              <pre className="text-sm">
                {`// 회전된 도형의 4개 꼭짓점 계산
...
const rotatedCorners = corners.map(corner => 
  localToGlobalCoordinates(corner.x, corner.y, rotation)
)
...
const maxY = Math.max(...rotatedCorners.map(c => c.y))`}
          ...
          <Box>
            ...
            <Text size="2">
              Shift 키를 누른 상태에서 리사이즈할 때 종횡비를 유지하려면 로컬
              좌표계에서 비율을 계산한 후 적용해야 합니다.
            </Text>
          </Box>
          ...
          <Box>
            ...
            <Text size="2">
              삼각 함수(cos, sin) 계산은 비용이 큽니다. 리사이즈 시작 시 한 번만
              계산하고 재사용하는 것이 좋습니다.
            </Text>
          </Box>
        </Box>
